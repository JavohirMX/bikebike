...
final class AppState: RaceSessionDelegate {
    ...
    func sessionDidFailToStart(error: Error) {
        ...
        if nsError.domain == NetService.errorDomain && nsError.code == -72008 {
            sessionErrorMessage = "Tap Try Again. If it keeps failing, delete and reinstall the app, tap Allow when prompted, or enable AR Racecar under Settings → Privacy & Security → Local Network."
        } else {
